...
if [ -f ${BASEDIR}/arm64.done ] && [ -f ${BASEDIR}/arm.done ] && [ -f ${BASEDIR}/x86.done ]; then
    ...
    if [ "$TREE_NAME" == "media" ]; then
        ...
        if [ "$BRANCH" == "master" ]; then
            curl -X POST -H "Authorization: $EMAIL_AUTH_TOKEN" -H "Content-Type: application/json" -d '{"job": "'$TREE_NAME'", "kernel": "'$GIT_DESCRIBE'", "git_branch": "'$BRANCH'", "build_report": 1, "send_to": ["ana.guerrero@collabora.com", "ezequiel@collabora.com", "guillaume.tucker@collabora.com"], "format": ["txt"], "delay": 10}' ${API}/send

            curl -X POST -H "Authorization: $EMAIL_AUTH_TOKEN" -H "Content-Type: application/json" -d '{"job": "'$TREE_NAME'", "kernel": "'$GIT_DESCRIBE'", "git_branch": "'$BRANCH'", "boot_report": 1, "send_to": ["ana.guerrero@collabora.com", "ezequiel@collabora.com", "guillaume.tucker@collabora.com"], "format": ["txt"], "delay": 5400}' ${API}/send

            curl -X POST -H "Authorization: $EMAIL_AUTH_TOKEN" -H "Content-Type: application/json" -d '{"job": "'$TREE_NAME'", "kernel": "'$GIT_DESCRIBE'", "git_branch": "'$BRANCH'",  "report_type": "test", "plans": ["v4l2"], "send_to": ["ana.guerrero@collabora.com", "ezequiel@collabora.com", "guillaume.tucker@collabora.com"], "format": ["txt"], "delay": 5400}' ${API}/send
        fi
    elif [ "$TREE_NAME" == "gtucker" ]; then
        if [ "$BRANCH" == "kernelci-media" ]; then
        echo "Sending results to Guillaume & Co"
        curl -X POST -H "Authorization: $EMAIL_AUTH_TOKEN" -H "Content-Type: application/json" -d '{"job": "'$TREE_NAME'", "kernel": "'$GIT_DESCRIBE'", "git_branch": "'$BRANCH'", "build_report": 1, "format": ["txt", "html"], "send_to": ["guillaume.tucker@collabora.com", "ana.guerrero@collabora.com"], "delay": 60}' ${API}/send
        curl -X POST -H "Authorization: $EMAIL_AUTH_TOKEN" -H "Content-Type: application/json" -d '{"job": "'$TREE_NAME'", "kernel": "'$GIT_DESCRIBE'", "git_branch": "'$BRANCH'", "boot_report": 1, "format": ["txt", "html"], "send_to": ["guillaume.tucker@collabora.com", "ana.guerrero@collabora.com"], "delay": 1800}' ${API}/send
